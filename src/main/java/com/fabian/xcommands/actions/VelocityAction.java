...
public class VelocityAction implements Action {
    ...
    public void execute(Player player, Map<String, Object> context) {
        ...
        try {
            ...
            com.fabian.xcommands.utils.LoggerUtils.debug("Attempting to send player " + player.getName() + " to server (Velocity): " + params.trim());
            player.sendPluginMessage(XCommands.getInstance(), "BungeeCord", out.toByteArray());
        } catch (Exception e) {
            com.fabian.xcommands.utils.LoggerUtils.severe("Failed to send player to velocity server: " + params, e);
        }
    }
